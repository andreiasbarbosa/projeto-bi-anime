...
Regras de recorte (definidas para manter o banco leve):
  1. Mantemos só os TOP_N animes com mais avaliações (default: 1000).
  2. Para cada um desses animes, amostramos no máximo SAMPLE_PER_ANIME
     avaliações aleatórias (default: 300) em vez de todas.
...
def build_dim_anime(anime_raw: pd.DataFrame, top_ids: set) -> pd.DataFrame:
    ...
    dim_anime = anime[
        ["MAL_ID", "Name", "Type", "estudio_principal", "ano", "Episodes", "Source", "Score", "Members", "Popularity"]
    ].rename(
        columns={
            "MAL_ID": "anime_id",
            "Name": "titulo",
    ...
    return dim_anime, anime  # devolve também o df completo p/ montar os gêneros
